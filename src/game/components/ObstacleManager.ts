...
export class ObstacleManager {
  ...
  constructor(scene: Phaser.Scene, obstacles: Phaser.Physics.Arcade.Group, waterLevel: number, gameSpeed: number) {
    this.scene = scene;
    this.obstacles = obstacles;
    this.waterLevel = waterLevel;
    this.gameSpeed = gameSpeed;
    this.nextObstacleTime = this.scene.time.now + Phaser.Math.Between(2000, 4000);
  }
